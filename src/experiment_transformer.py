"""
Transformer experiment for Sepsis Risk Prediction.
Encoder-only architecture with CLS token, causal attention mask,
learned missingness embeddings, cosine LR schedule, and early stopping.
"""
...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = self.input_norm(h)
        h = self.encoder(h)
        h = h[:, -1, :]  # Use last time step (like LSTM final hidden state)
...
def transform_sequences(
    ...
    """Median-impute and z-score normalize (same as LSTM preprocessing)."""
    X_filled = X.copy()
...
def predict_probabilities(model: TransformerClassifier, X: np.ndarray) -> np.ndarray:
    model.eval()
    with torch.no_grad():
...
def run_experiment(config: Config) -> Dict:
    ...
    model = train_transformer_model(X_train_norm, y_train, X_val_norm, y_val, config)
    ...
    results_json = out_dir / "transformer_results.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Transformer experiment for Sepsis Risk Prediction")
    ...
    parser.add_argument("--dim-feedforward", type=int, default=128, help="Feedforward dimension")
